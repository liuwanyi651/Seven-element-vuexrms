...
export default {
    ...
    mutations: {
        ...
        // 分类参数 参数列表
        setparaslistName(state, data) {
            state.paraslistName = data;
        },
        // 商品列表
    ...
    actions: {
        ...
        async getParameterListName({ commit }, params) {
            let res = await api.getParameterListName(params)
            if (res.meta.status === 200) {
                console.log(res.data);
                Message.success(res.meta.msg)
                commit('setparaslistName', res.data)
            }
        },
        // 添加动态参数或者静态属性
