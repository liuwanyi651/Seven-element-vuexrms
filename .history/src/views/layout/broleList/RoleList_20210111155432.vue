...
let { mapState: userState, mapMutations: userMatitons, mapActions: userActions } = userModule;
...
export default {
  ...
  methods: {
    ...
    showEidtDialog(row) {
      //拿到不同用户的id 30
      // // 调用 根据ID 查询用户信息的请求
      // this.getIdRolesMsg({ id: Number(id) });
      ...
      this.$refs.eidtFormRef.validate(valid=>{
        if (!valid) {
          return;
           ...
           console.log(15645,this.message);
          console.log(1111,this.message.roleId);
          // 可以发起修改用户的请求
          this.editRolesInfo({
            id:this.message.roleId,
            roleName:this.message.roleName,
            roleDes:this.message.roleName
          });
          // 关闭对话框
